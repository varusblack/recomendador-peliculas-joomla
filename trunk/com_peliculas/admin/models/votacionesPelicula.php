...
class PeliculasModelVotacionesPelicula extends JModel {
	...
	if ($puntuacion != 'no') {
	    ...
	    return $this->borrarVoto($idUsuario, $idPelicula);
	}
    }

    function borrarVoto($idUsuario, $idPelicula) {
	$db = &JFactory::getDbo();
	$query = "DELETE FROM #__votos WHERE idUsuario='{$idUsuario}' and idPelicula='{$idPelicula}'";
	$db->setQuery($query);
	$db->query();
...
// 		Devuelve: id,titulo,anno,tituloEspanol,director,puntuacion,timestamp
    ...
    function haVotado($idUsuario, $idPelicula) {
	$db = &JFactory::getDbo();
	$query = "SELECT count(*) as cuenta FROM #__votos WHERE idUsuario='{$idUsuario}' and idPelicula='{$idPelicula}'";
